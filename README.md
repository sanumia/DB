...
**Назначение таблицы:** Хранение рецептуры блюд. Определяет, какие ингредиенты и в каком количестве входят в каждое блюдо.

**Назначение полей:**
*   `dish_ingredient_id` — Уникальный идентификатор связи (первичный ключ)
*   `dish_id` — Ссылка на блюдо (внешний ключ)
*   `ingredient_id` — Ссылка на ингредиент (внешний ключ)
*   `quantity_required` — Норма расхода ингредиента на одну порцию блюда
1. Связи "Один ко многим" (1:М)
a) Справочники → Основные таблицы

positions → employees

Роль: Должность определяет роль сотрудника

Как работает: Одна должность может быть у многих сотрудников

order_statuses → orders

Роль: Статус определяет этап обработки заказа

Как работает: Один статус может быть у многих заказов

payments → orders

Роль: Способ оплаты определяет метод расчета

Как работает: Один способ оплаты может использоваться в многих заказах

categories → dishes

Роль: Категория группирует блюда меню

Как работает: В одной категории может быть много блюд

b) Сущности → Действия

clients → orders

Роль: Клиент является инициатором заказа

Как работает: Один клиент может сделать много заказов

employees → deliveries

Роль: Сотрудник выполняет доставку

Как работает: Один курьер может выполнить много доставок

2. Связи "Многие ко многим" (М:М) через таблицы-связки
a) Заказы ↔ Блюда (через order_items)

Что связывает: Состав заказа

Дополнительная информация:

Количество каждого блюда

Цена на момент заказа (исторические данные)

Бизнес-смысл: Фиксация фактического состава и стоимости заказа

b) Блюда ↔ Ингредиенты (через dish_ingredients)

Что связывает: Рецептуру блюд

Дополнительная информация:

Норма расхода ингредиента

Технологическая карта приготовления

Бизнес-смысл: Расчет себестоимости и управление складом

3. Связь "Один к одному" (1:1)
orders ↔ deliveries

Роль: Заказ и его логистика

Особенность: Уникальность связи (UNIQUE constraint)

Бизнес-смысл: Каждому заказу соответствует ровно одна попытка доставки
